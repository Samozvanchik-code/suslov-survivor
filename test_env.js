...
function поднять() {
  ...
  const файлы = ['config.js','text.js','content.js','sprites.js','engine.js','entities.js','ui.js'];
  for (const ф of файлы) {
    vm.runInContext(fs.readFileSync(__dirname + '/' + ф, 'utf8'), песочница, { filename: ф });
  }
  ...
  vm.runInContext(`globalThis.__ = {
    ИГРА, ЛУТ, БЕЛУГА, ЧЕРЕП, ГЕРОЙ, ОХОТНИК, СОЮЗНИКИ, АПГРЕЙДЫ, ВРАГИ, клавиши,
    начатьЗабег, шагЛогики, нарисоватьМир, нарисоватьИнтерфейс,
    обновитьИгрока, обновитьОружие, обновитьСвиту, ближайшийВрагК, обновитьПредметы, создатьВрага,
    ...
    пересчитатьМаксHp, ранитьИгрока, боевойМножитель
