...
/* Контекст с ЖУРНАЛОМ вызовов. Без него тест умеет проверять только «не
   упало»: рисование в заглушку ничего не оставляет. А проверять надо именно
   картинку — например, что донатер в клетке вообще нарисован, и что пол
   заливается на всю видимую при зуме область, а не по размеру холста.

   Журнал кольцевой на 400 записей: прогон test_soak рисует десятки тысяч
   кадров, и полный журнал съел бы память. */
function контекст() {
  const журнал = [];
  const записать = (з) => { журнал.push(з); if (журнал.length > 400) журнал.shift(); };
  return {
    canvas: { width: 800, height: 450 },
    _журнал: журнал,
    _очиститьЖурнал(){ журнал.length = 0; },
    save(){}, restore(){}, beginPath(){}, closePath(){}, moveTo(){}, lineTo(){},
    bezierCurveTo(){}, quadraticCurveTo(){}, arc(){}, arcTo(){}, ellipse(){},
    rect(){}, roundRect(){}, setLineDash(){}, getLineDash(){ return []; },
    fill(){}, stroke(){}, clip(){}, strokeRect(){}, clearRect(){},
    fillRect(x, y, ш, в){ записать({ вид: 'заливка', x: x || 0, y: y || 0, ш: ш || 0, в: в || 0 }); },
    translate(){}, rotate(){}, scale(){}, setTransform(){}, resetTransform(){},
    drawImage(и, x, y){
      записать({ вид: 'картинка', ш: и && (и.width || и.ш) || 0,
                 в: и && (и.height || и.в) || 0, x: x || 0, y: y || 0 });
    },
    fillText(){}, strokeText(){},
...
function элемент(тег) {
  const э = {
    ...
    /* Контекст КЭШИРУЕТСЯ. В браузере getContext дважды отдаёт один и тот же
       объект; без кэша тест не увидит журнал того контекста, в который
       рисует игра. */
    getContext: () => (э._контекст = э._контекст || контекст()),
...
function поднять(опции) {
  ...
  vm.runInContext(`globalThis.__ = {
    ...
    ВОЛНЫ, ТЕЛЕГРАФ, СНАРЯД, МАНИФЕСТ, СПРАЙТ, МАНИФЕСТ_ЗВУКОВ, ГОЛОС_КАРТЫ, проиграть, вызватьБосса, проверитьБоссов, обновитьБосса, завестиТелеграф,
    ...
    // пол под зумом, раскладка итогов, экран мирового топа, клетка донатера
    областьПола, масштабИтогов, нарисоватьПол,
    экранТопа, открытьТоп, закрытьТоп, прокрутитьСписок, топВМеню,
    переключитьТоп, видТопа, значениеВида, ВИДЫ_ТОПА,
    ДОНАТЕРЫ, запуститьИвент, обновитьКлетку, подрезать,
    // плашка ивента, бесконечка, скины и колесо
    обновитьОбъяву, нарисоватьОбъяву, ОБЪЯВА_ИВЕНТА,
    экранПобеды, предложитьПродолжение, продолжитьЗабег, времяЗаписи, ФИНАЛ,
    СКИНЫ, РЕДКОСТИ, КОЛЕСО, загрузитьШкаф, сохранитьШкаф, скинГотов, открытЛи,
    выбранныйСкин, спрайтИгрока, спрайтОружия, надетьСкин, датьПрокрут,
    пулКолеса, крутануть, обменятьДубли,
    открытьКолесо, закрытьКолесо, экранКолеса, открытьШкаф, закрытьШкаф, экранШкафа,
    хранЧитать, хранПисать,
    // громкость, музыка, свита
    ГРОМКОСТЬ, загрузитьГромкость, сохранитьГромкость, задатьГромкость,
    применитьГромкость, обновитьМузыку, нуженВидМузыки, МУЗЫКА,
    ТАРАКАНЫ, уровеньСвиты, обновитьТараканов, нарисоватьТаракана,
    обновитьКнопкуЗвука, переключитьПанельЗвука
